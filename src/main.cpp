...
#if defined(NDEBUG)
# error "Coinxinha cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbd9c3df942bada432eb19af3d91b1cd6e1b59cad784fa69ab256dd7551f11df0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coinxinha: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coinxinha Signed Message:\n";
...
{
    // Coinxinha: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Coinxinha: 840k blocks in ~4 years - 2 anos

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Coinxinha: 3.5 days - 1.5
static const int64 nTargetSpacing = 1.5 * 60; // Coinxinha: 2.5 minutes - 1.5
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coinxinha: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coinxinha: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbd9c3df942bada432eb19af3d91b1cd6e1b59cad784fa69ab256dd7551f11df0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12 de dezembro de 2015 Casamento de Welker e Chayene";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041218710fa689a12122015c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1477172223;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2666072;
        ...
        assert(block.hashMerkleRoot == uint256("0x9047c879a6925ea86902b2945328f7e14a550f8820734f01405394bb672dd46c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Coinxinha: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoinxinhaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoinxinhaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoinxinhaMiner(CWallet *pwallet)
{
    printf("CoinxinhaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coinxinha-miner");
    ...
    try { loop {
        ...
        printf("Running CoinxinhaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoinxinhaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoinxinhaMiner, pwallet));
}
